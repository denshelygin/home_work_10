...
def helper(message):
    bot.send_message(message.chat.id, 'Список команд для работы со справочником (в кавычках примеры):')
    bot.send_message(message.chat.id,'"/all" - показать весь справочник')
...
def add_C(message):
    ...
    try:
        ...
        f.save(BD)
        bot.send_message(message.chat.id, 'Контакт успешно добавлен в справочник')
    except:
